...
import sys
sys.path.append("./env/PriceForecastModel")
...
class PriceForecastModel:
    ...
    def select_device(self):
        """
        Selects GPU device based on availability and memory usage.
        If multiple GPUs are available, choose the second GPU if the first one has >50% memory usage.
        """
        if torch.cuda.is_available() and torch.cuda.device_count() > 1:
            from pynvml import nvmlInit, nvmlDeviceGetHandleByIndex, nvmlDeviceGetUtilizationRates
            nvmlInit()
            first_gpu_handle = nvmlDeviceGetHandleByIndex(0)  # First GPU
            first_gpu_util = nvmlDeviceGetUtilizationRates(first_gpu_handle).gpu  # GPU utilization percentage

            print(f"First GPU Utilization: {first_gpu_util}%")

            if first_gpu_util > 50:  # If GPU utilization > 50%, switch to the second GPU
                print("First GPU is over 50% utilized, switching to second GPU.")
                return torch.device("cuda:1")

            # Default to first GPU or CPU if no GPU is available
    ...
    def eval_episode(self, X):
        """
        :param X: Input data with shape (batch, seq_len)
        :return: y_pred: Predicted values with shape (batch, output_window)
        """
        X = torch.tensor(X, dtype=torch.float).to(self.device)
        X = X.unsqueeze(-1)
        with torch.no_grad():
            y_pred = self.model(X)
        return y_pred.cpu().numpy().squeeze().tolist()
